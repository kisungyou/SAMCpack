...
#' @keywords internal
#' @noRd
optionlist.optim <- function(options, nv, fname){
  parnames = names(options)
  if (!("domain" %in% parnames)){    domain = c(-Inf,Inf)  } else {domain=options$domain}
  if (!("partition" %in% parnames)){ partition = seq(-1e+2,1e+2,length.out=10)} else {partition=options$partition}
  if (!("vecpi" %in% parnames)){     vecpi=rep(1/9,9)} else {vecpi=options$vecpi}
  ...
  if (!("stepsize" %in% parnames)){  stepsize=1.0} else {stepsize=options$stepsize}
  if (!("trange" %in% parnames)){    trange=c(-Inf,Inf)} else {trange=options$trange}
  if (!("temp.tau.h") %in% parnames){temp.tau.h=1.0} else {temp.tau.h=options$temp.tau.h}
  if (!("temp.tau.star") %in% parnames){temp.tau.star=1.0} else {temp.tau.star=options$temp.tau.star}
  if (!("temp.t0") %in% parnames){temp.t0 = 1.0} else {temp.t0 = options$temp.t0}
  if (!("trunc.init")%in%parnames){trunc.init=10^10} else {trunc.init = options$trunc.init}
  if (!("trunc.r")%in%parnames){trunc.r=10^5} else {trunc.r = options$trunc.r}
  ...
  ## additional parameters for SAMCoptim
  temp.tau.h = as.double(temp.tau.h)
  if ((!is.numeric(temp.tau.h))||(temp.tau.h<=0)||(length(xi)>1)||(is.null(xi))){
    stop(cat("* ",fname," : 'temp.tau.h' should be in a positive real number.",sep=""))
  }
  temp.tau.star = as.double(temp.tau.star)
  if ((!is.numeric(temp.tau.star))||(temp.tau.star<=0)||(length(xi)>1)||(is.null(xi))){
    stop(cat("* ",fname," : 'temp.tau.star' should be in a positive real number.",sep=""))
  }
  temp.t0 = as.double(temp.t0)
  if ((!is.numeric(temp.t0))||(temp.t0<=0)||(length(xi)>1)||(is.null(xi))){
    stop(cat("* ",fname," : 'temp.t0' should be in a positive real number.",sep=""))
  }
  trunc.init = as.double(trunc.init)
  if ((!is.numeric(trunc.init))||(trunc.init<=0)||(length(xi)>1)||(is.null(xi))){
    stop(cat("* ",fname," : 'trunc.init' should be in a positive real number.",sep=""))
  }
  trunc.r = as.double(trunc.r)
  if ((!is.numeric(trunc.r))||(trunc.r<=0)||(length(xi)>1)||(is.null(xi))){
    stop(cat("* ",fname," : 'trunc.r' should be in a positive real number.",sep=""))
  }
  ...
  pruned$trange = trange
  pruned$m = m
  pruned$temp.tau.h = temp.tau.h
  pruned$temp.tau.star = temp.tau.star
  pruned$temp.t0 = temp.t0
  pruned$trunc.init = trunc.init
  pruned$trunc.r = trunc.r
