...
* First input argument is a `arma::vec` object from [RcppArmadillo](https://cran.r-project.org/web/packages/RcppArmadillo/index.html), representing random variable $x$.
...
### Why RcppArmadillo?
[RcppArmadillo](https://cran.r-project.org/web/packages/RcppArmadillo/index.html) is a templated C++ linear algebra library. We believe it balances *ease of use* like other script computing platform and *computational efficiency*. For example, most of expressions many statisticians often use involve some degree of numerical operations and RcppArmadillo is definitely one of the optimal options if not the best. For more details about the library, please see the introductory [vignette](https://cran.r-project.org/web/packages/RcppArmadillo/vignettes/RcppArmadillo-intro.pdf).
