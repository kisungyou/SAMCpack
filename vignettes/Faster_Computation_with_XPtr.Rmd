---
title: "Faster Computation with XPtr"
author: "Kisung You"
date: "`r Sys.Date()`"
output:
  pdf_document:
    fig_caption: true
vignette: >
  %\VignetteIndexEntry{Faster Computation with XPtr}
...
```
Second, let's assume we have a matrix-valued data, say `data_mat` is
$$
\begin{pmatrix}
20 & 20 & 10 \\
10 & 10 & 20
...
double x1 = x(0);
double x2 = x(1);
double val1 = (-std::pow((x1*sin(data(0,0)*x2)
              +x2*sin(data(0,1)*x1)),2))*cosh(sin(data(0,2)*x1)*x1);
double val2 = (-std::pow((x1*cos(data(1,0)*x2)
              -x2*sin(data(1,1)*x1)),2))*cosh(cos(data(1,2)*x2)*x2);
return Rcpp::wrap(val1+val2);
}"
```
...
double val1 = (-std::pow((x1*sin(lefty[0]*x2)
              +x2*sin(right(0,0)*x1)),2))*cosh(sin(right(0,1)*x1)*x1);
double val2 = (-std::pow((x1*cos(lefty[1]*x2)
               -x2*sin(right(1,0)*x1)),2))*cosh(cos(right(1,1)*x2)*x2);
return Rcpp::wrap(val1+val2);
}"
```
```{r echo=FALSE}
